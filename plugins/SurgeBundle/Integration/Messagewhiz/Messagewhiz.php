...
/**
 * Messagewhiz API2 Wrapper Class
 *
 * This class is used to interface with the Messagewhiz API2 to send messages, manage contacts, retrieve messages from
...
class Messagewhiz
{
	const REQUEST_URL = 'https://sms.mmdsmart.com/api/3/';
	 ...
	 * @param $hash
	 */
	function __construct($apiKey, $campaignId, LoggerInterface $logger)
	{
		$this->apiKey = $apiKey;
	...
	private function _sendRequest($command, $params = array())
	{
		...
		$result = json_decode(json_encode($rawResponse));
	...
	private function _sendRequestCurl($command, $params)
	{
		...
		if ($rawResponse === false) {
			throw new Exception('Failed to connect to the Messagewhiz service: ' . $error);
		} elseif ($httpCode != 200) {
			throw new Exception('Bad response from the Messagewhiz service: HTTP code ' . $httpCode);
		}
	...
	public function sendSms($numbers, $message, $sender, $sched = null, $test = false, $receiptURL = null, $custom = null, $optouts = false, $simpleReplyService = false)
	{
		...
		$params = array(
			"name" => "Single SMS to " . implode(',', $numbers),
		...
		return $this->_sendRequest('Broadcast', $params);
	}
